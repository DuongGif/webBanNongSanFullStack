...
const sectionsData = {
  ...
  contact: [
    { name: 'MaContact', label: 'Mã Liên Hệ', type: 'text' },
    { name: 'HoTen', label: 'Họ Tên', type: 'text' },
    { name: 'Email', label: 'Email', type: 'email' },
    { name: 'NoiDung', label: 'Nội Dung', type: 'text' },
  ]
};

const readOnlySections = ['hoadon', 'chitiethoadon', 'cart', 'contact'];
...
export default function AddProductForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMessage('Có lỗi xảy ra khi gửi dữ liệu!');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-xl bg-white shadow-md rounded-lg p-8">
        ...
        {readOnlySections.includes(section) ? (
          <p className="text-red-600 font-semibold text-center">
            Bạn <strong>không có quyền thêm mới</strong> {section}. Vui lòng sử dụng chức năng <strong>sửa hoặc xóa</strong>.
          </p>
        ) : (
